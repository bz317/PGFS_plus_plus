...
def load_weights(location: Path):
    if not location.is_file():
        raise FileNotFoundError(f"sEH proxy weights not found: {location}")
    with gzip.open(location) as gz:
...
def default_weights_path() -> Path:
    ...
    return project_root() / "scoring" / "seh" / "bengio2021flow_proxy.pkl.gz"


def load_original_model(cache=True, location=None):
    if location is None:
        location = default_weights_path()
